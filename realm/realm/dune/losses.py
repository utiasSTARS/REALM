...
def unic_loss(
    ...
    dset_name: List[str],
    tdrop: TeacherDropping = TeacherDropping(method="none", p=0.0),
    lam_lcos: float = 0.5,
    lam_lsl1: float = 0.5,
    ...
    loss, tcoeffs = tdrop(loss_pt, dset_name)
...
def loss_per_teacher(
    ...
    for tname in teacher_output.keys():
        ...
        for ttype in tout_dict.keys():
            ...
            with torch.autocast(device_type=sout.device.type, dtype=torch.float32):
