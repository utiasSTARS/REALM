...
def get_layerscale(dim, init_values=None):
    return (
...
class Block(nn.Module):
    def __init__(
        ...
        self.ls1 = get_layerscale(dim, layerscale_init)
        ...
        self.ls2 = get_layerscale(dim, layerscale_init)
...
class NestedTensorBlock(Block):
    ...
    def forward(self, x_or_x_list, **kwargs):
        if isinstance(x_or_x_list, Tensor):
            ...
            raise ValueError(
                "Type of x_or_x_list ({}) is not recognized".format(type(x_or_x_list))
            )
